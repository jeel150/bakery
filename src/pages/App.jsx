...
function App() {
  ...
  const sweetStoryRef = useRef(null);
  const occasionsRef = useRef(null);
  const miniBitesRef = useRef(null);

  // Search query (set by Navbar)
  ...
  // Active tabs
  const [selectedSweetTab, setSelectedSweetTab] = useState(null);
  const [selectedOccasionTab, setSelectedOccasionTab] = useState(null);
  const [selectedMiniBitesTab, setSelectedMiniBitesTab] = useState(null);
  ...
  // Fetch products when searchQuery changes (or on mount when empty)
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // If searching, find first matching section and scroll there + set the active tab for that section
        if (searchQuery.trim() !== "" && data.length > 0) {
          const sweetNames = sweetStoryCategories.map(c => c.name);
          const occNames = occasionCategories.map(c => c.name);
          const miniNames = miniBitesCategories.map(c => c.name);

          const firstSweetProduct = data.find(p => sweetNames.includes(p.category));
          if (firstSweetProduct) {
            setSelectedSweetTab(firstSweetProduct.category);
            setTimeout(() => { sweetStoryRef.current?.scrollIntoView({ behavior: "smooth" }); }, 150);
            return;
          }

          const firstOccProduct = data.find(p => occNames.includes(p.category));
          if (firstOccProduct) {
            setSelectedOccasionTab(firstOccProduct.category);
            setTimeout(() => { occasionsRef.current?.scrollIntoView({ behavior: "smooth" }); }, 150);
            return;
          }

          const firstMiniProduct = data.find(p => miniNames.includes(p.category));
          if (firstMiniProduct) {
            setSelectedMiniBitesTab(firstMiniProduct.category);
            setTimeout(() => { miniBitesRef.current?.scrollIntoView({ behavior: "smooth" }); }, 150);
            return;
          }

          // fallback to Just In if none matched the categorized sections
          setTimeout(() => { justInRef.current?.scrollIntoView({ behavior: "smooth" }); }, 150);
        }
      } catch (error) {
    ...
    fetchProducts();
    // include category arrays so scroll logic finds categories when they load
  }, [searchQuery, sweetStoryCategories, occasionCategories, miniBitesCategories]);
  ...
  // Likes
  const handleHeartClick = (productId, e) => {
    e.stopPropagation();
    setProductLikes(prev => ({ ...prev, [productId]: !prev[productId] }));
    setProductAnimations(prev => ({ ...prev, [productId]: true }));
    setTimeout(() => {
      setProductAnimations(prev => ({ ...prev, [productId]: false }));
    }, 400);
  };
  ...
  switch (section) {
    case "sweet-story":
      sweetStoryRef.current?.scrollIntoView({ behavior: "smooth" });
      break;
    case "occasions":
      occasionsRef.current?.scrollIntoView({ behavior: "smooth" });
      break;
    case "mini-bites":
      miniBitesRef.current?.scrollIntoView({ behavior: "smooth" });
      break;
    case "just-in":
      justInRef.current?.scrollIntoView({ behavior: "smooth" });
      break;
    default:
  ...
  return (
    <>
      ...
      <div className="position-relative w-100 main-container" style={{ overflowX: 'hidden', minHeight: '100vh' }}>
        ...
        {/* Just In Section (attach ref) */}
        <div className="justin-section" ref={justInRef}>
        ...
        {/* Sweet Story Section */}
        <div className="sweet-story-section" id="sweet-story-section" ref={sweetStoryRef}>
          ...
          >
            {products
              .filter(product => product.category === selectedSweetTab)
              .map((product) => (
                <div
                  key={product._id}
        ...
        {/* Imagination Section */}
        <div className="imagination-section" id="imagination-section">
        ...
        <div className="statement-cakes-section d-flex">
          ...
          <div className="statement-cakes-right d-grid">
            {products.slice(0, 8).map((p) => (
              ...
              >
                <div
                  className={`statement-card-heart position-absolute d-flex align-items-center justify-content-center${productLikes[p._id] ? ' liked' : ''}${productAnimations[p._id] ? ' animate' : ''}`}
                ...
                <img src={p.image} alt={p.name} className="statement-cake-img w-100" />
        ...
        {/* Occasions Section (attach ref) */}
        <div className="occasions-section " id="occasions-section" ref={occasionsRef}>
          ...
          >
            {products
              .filter(p => {
                if (!selectedOccasionTab || !p.category) return false;
                return p.category.toLowerCase() === selectedOccasionTab.toLowerCase();
              })
              .map((p) => (
                <div
        ...
        {/* Mini Bites Section (attach ref) */}
        <div className="minibites-section" id="minibites-section" ref={miniBitesRef}>
          ...
          >
            {products
              ...
              .map((p) => (
                ...
                >
                  <div
                    className={`minibites-card-heart position-absolute d-flex align-items-center justify-content-center${productLikes[p._id] ? " liked" : ""}${productAnimations[p._id] ? " animate" : ""}`}
                    onClick={(e) => handleHeartClick(p._id, e)}
                    style={{ cursor: "pointer", top: "10px", right: "10px" }}
                  >
                    <svg
        ...
        <div className="rb-footer-top d-flex">
          ...
          <div className="rb-footer-links d-flex">
            ...
            <div className="rb-footer-col d-flex flex-column">
             ...
             <ul className="list-unstyled">
  <li onClick={() => { setSelectedSweetTab("Custom Cake"); scrollToSection("sweet-story"); }} style={{ cursor: "pointer" }}>
    Customized Cakes
  </li>
  <li onClick={() => { setSelectedMiniBitesTab("Masson jar Cake"); scrollToSection("sweet-story"); }} style={{ cursor: "pointer" }}>
    Mason Jar Cakes
  </li>
  <li onClick={() => { setSelectedMiniBitesTab("Cake slice"); scrollToSection("mini-bites"); }} style={{ cursor: "pointer" }}>
    Cake Slice
  </li>
  <li onClick={() => { setSelectedMiniBitesTab("Brownie"); scrollToSection("mini-bites"); }} style={{ cursor: "pointer" }}>
    Brownies
  </li>
  <li onClick={() => { setSelectedMiniBitesTab("Cup Cake"); scrollToSection("occasions"); }} style={{ cursor: "pointer" }}>
    Cup Cake
  </li>
</ul>
